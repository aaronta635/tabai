#!/usr/bin/env python3
"""Light audio metrics for a student take. Stdlib only — librosa comes later."""
...
def metrics(samples: list[float], rate: int) -> dict:
    hop_s = 0.02
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps(metrics(samples, rate), ensure_ascii=True))
        return 0
    finally:
        try:
            os.remove(wav_path)
